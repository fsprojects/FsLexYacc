...
let ProcessParserSpecAst (spec: ParserSpec) =
    ...
    let terminalNamesSet = Set.ofList (terminals |> List.map fst)
    let IsTerminal z = terminalNamesSet.Contains(z)
    ...
    let mkSym ((name, range) as ident) = if IsTerminal name then Terminal ident else NonTerminal ident
    let prods =
                ...
                let precInfo =
                    let precsym =
                        ...
                        ||> List.foldBack (fun x acc ->
                            ...
                            | None ->
                                match x with
                                | (name, _range) as z when IsTerminal name -> Some z
                                | _ -> acc
                        )
    ...
    let nonTerminalNamesSet = Set.ofList (nonTerminals |> List.map fst)
    ...
    for (Production(nt,_,syms,_)) in prods do
        for sym in syms do
           ...
           | NonTerminal nt ->
               checkNonTerminal nt
           | Terminal (name, range) ->
               if not (IsTerminal name) then failwithf "%s(%d,%d) token %s is not declared" range.startPos.FileName range.startPos.Line range.startPos.Column name
