...
let ProcessParserSpecAst (spec: ParserSpec) =
    ...
    let prec_of_terminal sym implicitPrecInfo =
        if explicitPrecInfo.ContainsKey(sym) then
            explicitPrecInfo.[sym]
        else
            match implicitPrecInfo with
            | Some x -> x
            | None -> implicitSymPrecInfo

    let mkSym s =
        if IsTerminal s then Terminal s else NonTerminal s

    let prods =
        spec.Rules
        |> List.mapi (fun _ (nonterm, rules) ->
            rules
            |> List.mapi (fun _ (Rule(syms, precsym, code)) ->
                let precInfo =
                    let precsym =
                        List.foldBack
                            (fun x acc ->
