...
let logFileName (input: string, out: string option, log: bool) =
    if log then
        Some(
            match out with
            | Some x -> chop_extension x + ".fsyacc.output"
            | _ -> chop_extension input + ".fsyacc.output"
        )
    else
        None
...
let deriveOutputFileNames (filename, out: string option) =
    let output =
        match out with
        | Some x -> x
        | _ -> chop_extension filename + (if checkSuffix filename ".mly" then ".ml" else ".fs") in

    let outputi =
        match out with
        | Some x -> chop_extension x + (if checkSuffix x ".ml" then ".mli" else ".fsi")
        | _ ->
            chop_extension filename
            + (if checkSuffix filename ".mly" then ".mli" else ".fsi") in
...
type FileLogger(outputFileLog) =
    ...
    interface IDisposable with
        member _.Dispose() = osl.Dispose()

type NullLogger() =
    interface Logger with
        member x.LogStream f = f TextWriter.Null
        member x.Log f = fprintfn TextWriter.Null f
...
type Writer(outputFileName, outputFileInterface) =
    ...
    member x.WriteInterface format = fprintf osi format
...
let writeSpecToFile (generatorState: GeneratorState) (spec: ParserSpec) (compiledSpec: CompiledSpec) =
    ...
    if
        (generatorState.light = Some(false))
        || (generatorState.light = None && checkSuffix output ".ml")
    then
    ...
    spec.Tokens
    |> List.iteri (fun i (id, _) -> writer.WriteLine "  | %d -> TOKEN_%s " i id)
    ...
    compiledSpec.prods
    |> Array.iteri (fun i (nt, _, _, _) -> writer.WriteLine "    | %d -> NONTERM_%s " i nt)
    ...
    let getType nt =
        if types.ContainsKey nt then
            types.[nt]
        else
            generatorState.generate_nonterminal_name nt
    ...
    (writer.WriteLine "let _fsyacc_reductions = lazy [|"
     ...
     for nt, ntIdx, syms, code in compiledSpec.prods do
         ...
         |> List.iteri (fun i sym ->
             let tyopt =
                 match sym with
                 | Terminal t -> if tokens.ContainsKey t then tokens.[t] else None
    ...
    for id in spec.StartSymbols do
        ...
        writer.WriteLineInterface
            "val %s : (%s.LexBuffer<%s> -> token) -> %s.LexBuffer<%s> -> (%s) "
            id
            generatorState.lexlib
