...
let usage =
    [
        ...
        ArgInfo(
            "--open",
            ArgType.String(fun s -> opens <- opens @ [ s ]),
        ...
        ArgInfo(
            "--ml-compatibility",
            ArgType.Unit(fun _ -> compat <- true),
        ...
        ArgInfo(
            "--lexlib",
            ArgType.String(fun s -> lexlib <- s),
            "Specify the namespace for the implementation of the lexer (default: FSharp.Text.Lexing)"
        )
        ArgInfo(
            "--parslib",
            ArgType.String(fun s -> parslib <- s),
            "Specify the namespace for the implementation of the parser table interpreter (default: FSharp.Text.Parsing)"
        )
        ArgInfo(
            "--codepage",
            ArgType.Int(fun i -> inputCodePage <- Some i),
...
let _ =
    ArgParser.Parse(
        ...
        (fun x ->
            match input with
            | Some _ -> failwith "more than one input given"
            | None -> input <- Some x),
        "fsyacc <filename>"
    )
...
let main () =
    ...
    use logger =
        match logFileName (filename, out, log) with
